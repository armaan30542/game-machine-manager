...
import { addCabinetType, deleteCabinetType } from "@/actions/cabinet-type-actions";
...
function CabinetTypesManager() {
  const { data: types = [], isLoading } = useCabinetTypes();
  const [newType, setNewType] = useState("");
  const [adding, setAdding] = useState(false);
  const [typeSearch, setTypeSearch] = useState("");
  const [deleteTarget, setDeleteTarget] = useState<string | null>(null);
  ...
  async function handleAdd() {
    if (!newType.trim()) return;
    setAdding(true);
    const result = await addCabinetType(newType);
    if (result.error) {
      toast.error(result.error);
    } else {
      toast.success(`Added "${newType.trim()}"`);
      setNewType("");
      queryClient.invalidateQueries({ queryKey: ["cabinet-types"] });
    }
    setAdding(false);
  }
  ...
  async function handleDelete() {
    if (!deleteTarget) return;
    setDeleting(true);
    const result = await deleteCabinetType(deleteTarget);
    if (result.error) {
      toast.error(result.error);
    } else {
      toast.success(`Deleted "${deleteTarget}"`);
    ...
    <>
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Manage the list of cabinet types available in the dropdown when creating or editing machines.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex gap-2">
            <Input
              placeholder="New cabinet type name..."
              value={newType}
              onChange={(e) => setNewType(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && handleAdd()}
            />
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Cabinet Type</AlertDialogTitle>
            <AlertDialogDescription>
              Delete &quot;{deleteTarget}&quot;? This will only succeed if no
              machines currently use this cabinet.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
