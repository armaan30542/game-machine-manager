...
export function LocationDetailClient({ id }: { id: string }) {
  ...
  const { location, machines, dispenser, inventoryMachines, inventoryDispensers } = data;

  if (!location) {
    return (
      <div className="text-center py-12">
        <p className="text-muted-foreground">Location not found</p>
        <Link href="/locations" className="text-primary hover:underline text-sm">
          Back to locations
        </Link>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="font-medium">{machines.length}</p>
              </div>
            </div>
      ...
      <LocationMachines
        locationId={id}
        machines={machines}
        inventoryMachines={isAdmin ? inventoryMachines : []}
        inventoryDispensers={isAdmin ? inventoryDispensers : []}
        dispenser={dispenser}
        isAdmin={isAdmin}
